...
import type { GetAnswerTitleHistoryResponse } from '@/lib/api-types';

const EMPTY_TITLE_HISTORY_PAGE: GetAnswerTitleHistoryResponse = {
  items: [],
  next_cursor: null,
};

/**
 * 回答のタイトル変更履歴を全ページ取得する。enabled が false の間はリクエストしない
 * (履歴ダイアログを開くまで発火させないため)。
 */
export const useAnswerTitleHistory = (
  formId: string,
  answerId: string,
  enabled: boolean
) => {
