...
StorybookとJestの2種類のライブラリを用いてテストを記載しています。将来的には、CLIからどちらも1つのコマンドで実行できるようにする予定です。

* Storybook
  * `yarn sb:test`
  * UIコンポーネントは、基本的にStorybookを使用してカタログを作成しています。
  * 同時にテストを視覚的に実行できるので、これでテストできるものはユニットテストを記載しています。
* Jest
  * `yarn test`
  * それ以外のもののテストは全てJestを使用してテストを作成しています。
